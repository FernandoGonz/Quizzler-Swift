...
class ViewController: UIViewController {
    ...
    @IBAction func answerButtonPressed(_ sender: UIButton) {
        ...
        if quizBrain.questionNumber + 1 < quizBrain.getCount() {
    ...
    @objc func showQuestion() {
        questionLabel.text = quizBrain.getQuestion() // show the question from QuizBrain()
        trueButton.backgroundColor = UIColor.clear // to clear the background color
        falseButton.backgroundColor = UIColor.clear
        progressBar.progress = Float(quizBrain.questionNumber + 1) / Float(quizBrain.getCount()) //
    }
